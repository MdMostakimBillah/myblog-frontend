...
//                       qualification: user.qualification || "",
//                     });
//                     setDraftInfo((p) => ({ ...p, location: user?.location || "" }));
//                     setEditProfile(true);
//                   }}
//                 >
//                   <FaEdit size={12} />
...
export default function About() {
  ...
  // ✅ নতুন: দক্ষতা ও পাসওয়ার্ড modal
  ...
  const handleSkillsSave = async () => {
    const skillNames = draftSkills.map((s) => s.name).filter(Boolean);
    const ok = await saveField({ skills: skillNames });
    if (ok) setEditSkills(false);
  };

  // ✅ পাসওয়ার্ড save
  const handlePasswordSave = async () => {
    if (draftPassword.new1 !== draftPassword.new2) {
      toast.error("নতুন পাসওয়ার্ড দুটি মিলছে না!");
      return;
    }
    if (draftPassword.new1.length < 6) {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/auth/change-password`,
        {
          ...
          body: JSON.stringify({
            userId: user._id,
            currentPassword: draftPassword.current,
            newPassword: draftPassword.new1,
          }),
        }
      );
      const data = await res.json();
      if (data.success) {
        ...
        setDraftPassword({ current: "", new1: "", new2: "" });
      } else {
        toast.error(data.message || "পাসওয়ার্ড পরিবর্তন করতে সমস্যা হয়েছে");
      }
    } catch {
      toast.error("সার্ভারে সমস্যা হয়েছে");
    }
  ...
  // ✅ এই helper — location সহ draftInfo সেট করে editProfile খোলে
  const openProfileModal = (userData) => {
    ...
    // ✅ FIX: location এখন সঠিকভাবে সেট হচ্ছে
    setDraftInfo((p) => ({ ...p, location: userData?.location || "" }));
    setEditProfile(true);
  };
    ...
    <div className={css.page}>
      ...
      <div className={css.body}>
          ...
          <div className={css.section}>
            <div className={css.sectionHead}>
              ...
              {isOwnProfile && (
                <button
                  className={css.editIconBtn}
                  title="দক্ষতা পরিবর্তন করুন"
                  onClick={() => {
                    const current =
                      user?.skills?.length
                        ? user.skills.map((s) => ({ name: s }))
                        : SKILLS_DEFAULT.map((s) => ({ name: s.name }));
                    setDraftSkills(JSON.parse(JSON.stringify(current)));
                    setEditSkills(true);
                  }}
                >
                  <FaEdit size={13} />
        ...
        <div className={css.right}>
            ...
            <div className={css.cardHeadRow}>
              ...
              {isOwnProfile && (
                <button
                  className={css.editIconBtn}
                  title="তথ্য পরিবর্তন করুন"
                  // ✅ FIX: openProfileModal ব্যবহার করা হচ্ছে — location সেট হবে
                  onClick={() => openProfileModal(user)}
                >
                  <FaEdit size={12} />
                </button>
          ...
          {/* ✅ পাসওয়ার্ড পরিবর্তন বাটন */}
          {isOwnProfile && (
            <div className={css.card}>
              <div className={css.cardHeadRow}>
                <p className={css.cardTitle}>নিরাপত্তা</p>
              </div>
              <button
      ...
      {/* Profile Info Edit (নাম, location, email, শিক্ষা) */}
      {editProfile && (
        <Modal
          title="ব্যক্তিগত তথ্য পরিবর্তন করুন"
          onClose={() => setEditProfile(false)}
        ...
        >
          <div className={css.modalGrid}>
            ...
            {/* ✅ FIX: location এখন সঠিকভাবে pre-filled হচ্ছে */}
            <div className={css.modalField} style={{ gridColumn: "1 / -1" }}>
              <label className={css.modalLabel}>অবস্থান</label>
              <input
                className={css.modalInput}
        ...
        >
          <div className={css.tlEditList}>
              ...
              <div key={i} className={css.tlEditItem}>
                <div className={css.tlItemTop}>
                  ...
                  <button
                    className={css.tlRemoveBtn}
                    onClick={() =>
                      setDraftSkills((p) => p.filter((_, idx) => idx !== i))
                    }
                  >
                    ✕
                  </button>
                ...
                <div className={css.modalField}>
                  <label className={css.modalLabel}>দক্ষতার নাম</label>
                  <input
                    className={css.modalInput}
                    value={s.name}
                    onChange={(e) =>
            ...
            <button
              className={css.tlAddBtn}
              onClick={() =>
                setDraftSkills((p) => [...p, { name: "" }])
              }
            >
              + নতুন দক্ষতা যোগ করুন
            </button>
          </div>
        </Modal>
      )}

      {/* ✅ পাসওয়ার্ড পরিবর্তন Modal */}
      {editPassword && (
        <Modal
        ...
        >
          <div className={css.modalGrid}>
            <div className={css.modalField} style={{ gridColumn: "1 / -1" }}>
              <label className={css.modalLabel}>বর্তমান পাসওয়ার্ড</label>
              <input
                type="password"
                className={css.modalInput}
                value={draftPassword.current}
                onChange={(e) =>
                  setDraftPassword((p) => ({ ...p, current: e.target.value }))
                }
                placeholder="বর্তমান পাসওয়ার্ড দিন"
              />
            </div>
            <div className={css.modalField} style={{ gridColumn: "1 / -1" }}>
              <label className={css.modalLabel}>নতুন পাসওয়ার্ড</label>
              <input
                type="password"
                className={css.modalInput}
                value={draftPassword.new1}
                onChange={(e) =>
                  setDraftPassword((p) => ({ ...p, new1: e.target.value }))
                }
                placeholder="নতুন পাসওয়ার্ড দিন (কমপক্ষে ৬ অক্ষর)"
              />
            </div>
            <div className={css.modalField} style={{ gridColumn: "1 / -1" }}>
              <label className={css.modalLabel}>নতুন পাসওয়ার্ড নিশ্চিত করুন</label>
              <input
                type="password"
                className={css.modalInput}
                value={draftPassword.new2}
                onChange={(e) =>
                  setDraftPassword((p) => ({ ...p, new2: e.target.value }))
                }
